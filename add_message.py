...
async def go_message(client, message):
    ...
    if message.content.startswith("#문의봇명령어"):
        embed = discord.Embed(title='문의봇 명령어 안내', color=0x00ff48, timestamp=message.created_at)
        ...
        embed.set_footer(text="Uiwang-si", icon_url="https://cdn.discordapp.com/avatars/727076241777229854/fd20bae7647ba1aa6919d515c44f5287.png?size=128")
    ...
    ############################################ 문의 종료 부분 ############################################
        ...
        if not channel:
            ...
            embed = discord.Embed(title='의왕시 고객센터', description=f"{user.name}님 문의가 종료되었습니다 \n\n> 다시 문의하고 싶으신 경우에만 메시지를 다시 보내주세요\n\n > 앞으로 더욱 나아가는 시그널서버가 되겠습니다 :)", color=0x00ff48, timestamp=message.created_at)
            embed.set_footer(text='의왕시 고객센터', icon_url=client.user.avatar_url)
            await user.send(embed=embed)
            now = datetime.now()
            embed = discord.Embed(title='{}'.format(now.strftime("%Y - %m - %d")), color=0x00ff48, timestamp=message.created_at)
            embed.add_field(name="문의 종료 로그", value=f"문의한 유저 : <@{channel}>\n\n문의 종료한 관리자 : <@{message.author.id}>", inline=True)
            embed.set_footer(text='Uiwang-si', icon_url=client.user.avatar_url)
    ...
    ############################################ 문의 답변 부분 ############################################
        ...
        if user_id:
            ...
            if message.attachments:
                ...
                await user.send(embed=discord.Embed(title="문의 답변", description=f"의왕시 고객센터 : {message.content}", color=0x00ff48))
                await user.send(message.attachments[0].url)
                open(f"log/{user.id}.txt", 'a', encoding='utf-8 sig').write(f"의왕시 고객센터({message.author.name }) : {message.content} , {message.attachments[0].url}\n")
            else:
                await message.add_reaction('👻')
                await user.send(embed=discord.Embed(title="문의 답변", description=f"의왕시 고객센터 : {message.content}", color=0x00ff48))
                open(f"log/{user.id}.txt", 'a', encoding='utf-8 sig').write(f"의왕시 고객센터({message.author.name}) : {message.content}\n")

    ########################################문의 or 첫 문의 부분#########################################
    if message.guild is None:
        black, reason = config.check_black(message.author.id)
        if black == True:
            await message.channel.send(embed=discord.Embed(title="의왕시 고객센터", description=f"안녕하세요 {message.author.name}님 당신은 '{reason}'의 사유로 블랙리스트에 등록되어 문의가 불가능합니다", color=0xff0000))
        else:
            ...
            if close_user_m[message.author.id] == 1:
                return await message.channel.send(embed=discord.Embed(title="의왕시 고객센터", description=f"안녕하세요 {message.author.name}님 현재 전에 문의한 내용이 정리되고있습니다 잠시 후 다시 시도해주세요", color=0xff0000))
            if user_id == True: #새로운 문의자가 맞을경우
                ...
                if user_command_time[message.author.id] <= now:
                    if message.attachments:
                        ...
                        if not bool:
                            embed = discord.Embed(title='의왕시 고객센터', color=0x00ff48, timestamp=message.created_at)
                            embed.add_field(name="문의 접수", value=f"{message.author.name}님 문의접수가 완료되었습니다. 관리자의 응답을 기다려주세요\n\n 문의 내용 : {message.content}", inline=True)
                            embed.set_footer(text='의왕시 고객센터', icon_url=client.user.avatar_url)
                            embeda = discord.Embed(title='의왕시 고객센터', description="유저에게 문의가 도착했어요!" ,color=0x00ff48, timestamp=message.created_at)
                            ...
                            embeda.set_footer(text='의왕시 고객센터', icon_url="https://cdn.discordapp.com/attachments/1076414596723773480/1077914833162338335/-001_13.png")
                            await message.channel.send(embed=embed)
                        ...
                        await channel.send(embed=discord.Embed(title="문의", description=f"{user_nick} : {message.content}", color=0x00ff48))
